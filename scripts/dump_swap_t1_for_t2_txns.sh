...
./goal app call --app-id ${VALIDATOR_INDEX} --app-arg "str:s1" --app-arg "int:80888500" --from ${TEST_ACCOUNT_ADDRESS} --app-account ${ESCROW_ADDRESS} --out=txn1.tx

./goal app call --app-id ${MANAGER_INDEX} --app-arg "str:s1" --app-arg "int:80888500" --from ${TEST_ACCOUNT_ADDRESS} --app-account ${ESCROW_ADDRESS} --out=txn2.tx

./goal asset send -a 100000000 --assetid ${TOKEN1_INDEX} -f ${TEST_ACCOUNT_ADDRESS} -t ${ESCROW_ADDRESS} --out=txn3.tx
...
./goal clerk dryrun -t signout.tx --dryrun-dump --dryrun-dump-format json -o swap-t1-for-t2-dr.json